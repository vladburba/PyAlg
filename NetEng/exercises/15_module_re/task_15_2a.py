...
def convert_to_dict(d_keys, data):
    interface_list = []
    for k in data:
        interface_list.append(dict(zip(d_keys, k)))
    return interface_list


if __name__ == '__main__':
    headers = ['interface', 'address', 'status', 'protocol']
    values = task_15_2.parse_sh_ip_int_br('sh_ip_int_br.txt')
    print(headers)
    print(values)
    print(convert_to_dict(headers, values))
